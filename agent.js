...
class GyaanSetu {
    ...
    setupEventListeners() {
        try {
            ...
            // Apply sidebar state on load and window resize
            window.addEventListener('resize', () => this.updateSidebarState());
            this.updateSidebarState();

            // Global command input: press Enter to send
            this.elements.commandInput?.addEventListener('keydown', (e) => {
                if (e.key === 'Enter' && !e.shiftKey) {
                    ...
                    if (this.elements.userInput) {
                        this.elements.userInput.value = val;
                        this.updateCharCount();
                    }
    ...
    updateSidebarState() {
        ...
        // ensure mutually exclusive classes
    ...
    updatePerformanceDisplay() {
        try {
            const rt = `${this.state.performance.responseTime}ms`;
            const mem = `${this.state.performance.memoryUsage}MB`;
            const calls = this.state.performance.apiCalls || 0;
            ...
            if (this.elements?.statusResponse) this.elements.statusResponse.textContent = rt;
            if (this.elements?.statusMemory) this.elements.statusMemory.textContent = mem;
            if (this.elements?.statusApiCalls) this.elements.statusApiCalls.textContent = calls;
        } catch (e) {}
    }
